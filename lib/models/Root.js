...
export default class Root extends RouterElementMiddleware {
   ...
   */
  constructor(name, config, parserConfig, parser, extraParams = []) {
    super(Root.name, config, ['name', 'root', 'params', 'routes'])

    this.__parseRouteElements = parser
  ...
  __parseRoutes(routes, parserConfig) {
    ...
    this.routes = this.__parseRouteElements(routes, parserConfig, this.params)
  }
  ...
  toString() {
    ...
    const routes = [
      ...this.pre_middlewares, // prettier-ignore
      ...this.routes,
    ...
    for (const route of routes) {
      ...
      if (route instanceof Import) {
        ...
        body.push(firstChar + '── ' + padStartText(routeString, ' ', 4).slice(4))
      } else {
        body.push(firstChar + '── ' + routeString)
      }
    }
