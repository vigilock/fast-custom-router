...
export default class RouteParameter extends RouterElement {
  ...
  constructor(name, config) {
    super(RouteParameter.name, typeof config === 'string' ? {} : config, ['type', 'default_value'])
  ...
  valid(value) {
    ...
    try {
      ...
      throw new ValidationDataError(error.code, 'invalidParameter', `Parameter "${this.name}" cannot be valided (Reason: ${error.message})`)
    }
  }
}
