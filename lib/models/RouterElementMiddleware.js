...
export default class RouterElementMiddleware extends RouterElement {
   ...
   */
  constructor(childName, config, params = []) {
    super(childName, config, [...params, 'pre_middlewares', 'post_middlewares'])
  ...
  __parseMiddlewares(middlewares) {
    if (middlewares) {
      if (!(middlewares instanceof Array)) {
        throw new InvalidArgument(`${this.childName}.middlewares=${middlewares} is not an dictionnary.`)
      }
