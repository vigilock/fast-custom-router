...
export default class Parser {
  ...
  static __parseRouteElements(config, parserConfig, extraQuery = []) {
    const res = []
    Object.keys(config).forEach(key => {
      const el = config[key]
      if (key === 'import') {
        res.push(new Import(el, parserConfig, extraQuery))
      } else {
        if (el?.root) {
          res.push(new Root(key, el, parserConfig, extraQuery))
        } else if (el?.path) {
          res.push(new Route(key, el, extraQuery))
        } else {
